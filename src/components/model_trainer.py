                ...
                def finetune_best_model(self,
                       ...
                       try:
                            ...
                            gridsearch = GridSearchCV(
                                  best_model_object, param_grid,cv=5,n_jobs=-1,verbose=1)
    ...
    def  initiate_model_trainer(self,train_array,test_array):
                        ...
                        try:
                             ...
                             if best_model_score < 0.5:
                                  raise Exception("No best model found with an accurecy greater than the threshold 0.6")
                             
                             logging.info(f"Best found model on both training and testing dataset")


                             logging.info(
                                  f"saving model at path: {self.model_trainer_config.trainer_model_path}"

                             )

                             os.makedirs(os.path.dirname(self.model_trainer_config.trained_model_path),exit_ok=True)
