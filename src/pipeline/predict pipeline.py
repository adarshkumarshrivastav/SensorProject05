...
class PredictionPipelineConfig:
    ...
    preprocessor_path: str = os.path.join(artifacts_folder,'preprocessor.pkl')
...
def save_input_files(self) -> str:
    try:
        pred_files_inputs_dir = " prediction_artifacts"
        os.makedirs(pred_files_inputs_dir, exist_ok= True)

        input_csv_file = self.request.files['file']
        pred_file_path =os.path.join(pred_files_inputs_dir,input_csv_file.filename)
    ...
    def predict(self,features):
        ...
        def get_predicted_dataframe(self,input_dataframe_path: pd.Dataframe):
            try:
                ...
                os.makedirs(self.prediction_pipelines_config.prediction_output_dirname,exist_ok=True)
